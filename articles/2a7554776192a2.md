...
>
> Rocketのコアのリクエスト処理は、Rustの最新の非同期ネットワーク機能を利用するためにv0.5で再構築されました。tokioによってバックアップされたRocketは、マシン上で利用可能なすべてのコアの非同期タスク間でリクエスト処理を自動的に多重化します。その結果、ルートハンドラを非同期で宣言し、await構文を利用できるようになりました：
...
async fn main() {
    ...
    let app = Router::new()
        .route("/users", get(user_handler).layer(Extension(user_service)))    // Here
